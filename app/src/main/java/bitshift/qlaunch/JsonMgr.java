...
public class JsonMgr
{
    ...
    JsonMgr(Context context)
    {
        ...
        if (!rootExists)
        {
            Log.i(Globals.APP_NAME, "No previous groups found");
        ...
        {
            Log.i(Globals.APP_NAME, "Loading groups");
            loadGroups();
            loadApplications();
            loadShortcuts();
        }
    ...
    public void writeGroup(Group item)
    {
        try
        {
            ...
            object.put(COLUMN_CATEGORY, item.category());
    ...
    public void deleteItemFile(GroupItem item)
    {
        ...
        Log.i(Globals.APP_NAME, "Delete JSON:" + item.packageName());
        f.delete();
    ...
    public List<File> getFileList(String path)
    {
        ...
        if (files == null) {
            Log.e(Globals.APP_NAME, "Couldn't find any files in: " + path);
            return list;
        }
    ...
    public Application readApplication(UUID id)
    {
        ...
        try
        {
            ...
            if (object.has(COLUMN_LABEL))
                item.setLabel(object.getString(COLUMN_LABEL));

            if (object.has(COLUMN_CATEGORY))
                item.setCategory(object.getString(COLUMN_CATEGORY));

            if (object.has(COLUMN_FLAGS))
                item.setFlags(object.getLong(COLUMN_FLAGS));

            if (object.has(COLUMN_OVERRIDEPOSITION))
                item.setOverridePosition(object.getLong(COLUMN_OVERRIDEPOSITION));

            if (object.has(COLUMN_ICONOVERRIDE))
    ...
    public Group readGroup(UUID id)
    {
        ...
        try
        {
            ...
            {
                if (!item.testFlag(GroupItem.FLAG_ROOTGROUP))
                    Log.e(Globals.APP_NAME, "Group '" + item.label() + "' has no parent! JSON:" + json);
            }
        }
        catch (Exception e)
        {
